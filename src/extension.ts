...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('laravel-routes.helloWorld', () => {
		// The code you place here will be executed every time your command is executed
		// Display a message box to the user
		vscode.window.showInformationMessage('Hello World from laravel-routes!');
	});
	let start = vscode.commands.registerCommand('laravel-routes.start',(routesFolder:string)=>{
			LaravelRoutes.make().setRoutesPath('routes').start();
	});
